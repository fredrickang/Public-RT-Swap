...
bool exist_current_swap_task(task_list_t * task_list){
    for(task_info_t * task = task_list->head; task != NULL; task = task->next){
        if(task -> state == SWAPIN || task -> state == SWAPOUT){
            //DEBUG_PRINT(RED"Task(%d) Currently Swap\n"RESET, task->id);
            return true;    
        } 
    }
    return false;
...
int dequeue_asyncswap(char * que_name, queue_t *q, task_list_t *task_list, resource_t *res, resource_t * swap_in){
    ...
    node_t *target = q->front; // schedule target: highest priority task
    ...
    if(target_task->swap_curr != 0 && (MEM_LIMIT - mem_current) < target_task->swap_curr){
        //DEBUG_PRINT(RED"No space for target task(%d), Swap-out scheduled\n"RESET, target_task->id);
        ...
        //DEBUG_PRINT(RED"Updating queue\n"RESET);
        return -2;
    }
    ...
    if(target_task->swap_curr != 0){
        //DEBUG_PRINT(BLUE"Need Swap-In ID(%d)\n"RESET, target_task->id);
    ...
    free(target);
    //print_queue(que_name, q);
    //DEBUG_PRINT(RED"Task(%d) will be scheduled\n"RESET, target_pid);
    return target_pid;
...
void do_register(task_list_t *task_list, reg_msg *msg){
    ...
    // These are darknet based memory object profile results
    // Following objects are not used during inference time
    // We deallocate does objects for efficient GPU memory usage
...
void sch_request_handler(task_list_t *task_list, task_info_t *task, resource_t *res, resource_t *init_que, resource_t *swap_in, resource_t *warmup_que){    
    ...
    if(init_que -> state == BUSY && init_que -> pid == task -> pid){
        ...
        //remove_non_access_allocations(task_list, task);
    }
    ...
    if(res -> state == BUSY && res -> pid == task->pid){ /* Job termniation */
        ...
        if(task->deadline < current_time){
             ...
             //exit(-9); // Temprally disabled the deadline miss check log for profiling
         }
        res -> state = IDLE;
        res -> pid = -1;
        ...
        if(swap_in->waiting->count != 0 && !exist_current_swap_task(task_list)){
            ...
            if(highest_task->swap_curr > (MEM_LIMIT - mem_current) && task->swap_max != 0){
                ...
                //swapout(task_list, task, highest_task->swap_curr - (MEM_LIMIT - mem_current));
                //DEBUG_PRINT(GREEN"Swap Out Async(%d) amount: %10lu\n"RESET,task->pid, outted);
            }
            if(highest_task->swap_curr <= (MEM_LIMIT - mem_current)){
                swapin_async(task_list, highest_task);
                // swapin(task_list, task);
                dequeue("SwapIn",swap_in->waiting, swap_in);
            }
        }
        task-> job_count++;
...
bool exist_currently_running_task(task_list_t * task_list){
    for(task_info_t * task = task_list->head; task != NULL; task = task->next){
        if(task->state == SCHEDULED) {
            // DEBUG_PRINT(RED"Current Running Task(%d)\n"RESET,task->id);
            return true;
        }
    }
    return false;
...
cudaAPI mm_request_handler(task_list_t * proc_list, task_info_t * proc, resource_t * swap_in){
    ...
    //DEBUG_PRINT(RED"[MM] Task(%d) msg_type(%d)\n"RESET, proc->id, msg->type);
    if(msg->type == _SO_DONE_){
        DEBUG_PRINT(GREEN"%20s(%d) - %.4f"RESET,"Swap-out Done",proc->id, what_time_is_it_now());
        //fprintf(stderr,RED" / %3.4fms\n"RESET, (what_time_is_it_now() - proc->swap_time)*1000); // Swap out termination 
        
        
        proc->state = ALIVE;
        proc->swap_curr += msg->size; // update process swap amount 
        ...
        if(scheduler_state == 1){
            ...
            // this swap out done by overlapping
    ...
    if(msg->type == _SI_DONE_){
        ...
        DEBUG_PRINT(GREEN"%20s(%d) - %.4f"RESET,"Swap-in Done", proc->id, what_time_is_it_now());
        //fprintf(stderr,RED" / %3.4fms\n"RESET,(what_time_is_it_now() - proc->swap_time)*1000);  // Swap in termination
        DEBUG_PRINT(RED"Mem Current: %10lu, Left: %10lu\n"RESET, mem_current, MEM_LIMIT - mem_current);
        proc->swap_experince = 1;
    ...
    if(msg->type == _cudaMalloc_){
        ...
        if(exist_inside(proc->swap_allocations, proc->swap_num, requested_idx)){
            // DEBUG_PRINT("Mem Current: %lu\n", mem_current);
            // DEBUG_PRINT("Req size: %lu\n", msg->size);
            ...
            // DEBUG_PRINT("Before(%d) -> After (%d)\n", before_chunk_num, after_chunk_num);
            updated_mem_state = mem_current + (after_chunk_num - before_chunk_num)*MIN_CHUNK;
            // DEBUG_PRINT("Updated (%lu)\n", updated_mem_state);
        ...
        if(malloc_type != -1) fprintf(stderr, "%d, %lu\n",msg->entry_index, msg->size);

        // DEBUG_PRINT(RED"Malloc(%lu)-type(%d)-Before(%lu)-After(%lu)\n"RESET, msg->size, malloc_type, mem_current, updated_mem_state);
        ...
        //notify library which type of Malloc
        ...
        // DEBUG_PRINT(RED"Normal mem: %10lu\n"RESET,normal_mem);
        // DEBUG_PRINT(RED"VMM mem: %10lu\n"RESET,vmm_mem);
        return msg->type;
    }
...
void read_cfg(char * path, task_list_t *task_list){
    FILE * fp;
    fp = fopen(path, "r");
    int task_num = task_list->count;

    for(int i = 0; i < task_num; i++){
        int model_type;
        int period;
        int swap_num;
        fscanf(fp,"%d,%d,%d,",&model_type,&period,&swap_num);
        fprintf(stderr,"%d, %d, %d\n", model_type, period, swap_num);
        if(swap_num){
            task_info_t * task;
