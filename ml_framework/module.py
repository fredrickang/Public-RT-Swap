...
def scheduling_interface(self, input):
    global request_fd
    global decision_fd
    global release_time
    global sync
    ...
    if not sync:
        ack = 2
        request = [ack]
        brequest = msg2bmsg(request)
        os.write(request_fd, brequest)

    if release_time != None:
        release_time += self.period * 0.001
        current_time = time.clock_gettime(time.CLOCK_MONOTONIC)
        try:
            time.sleep(release_time - current_time)
        except:
            print("You miss the deadline....")
            print("Skip the next job")
            release_time += self.period * 0.001
            time.sleep(release_time - current_time)
    comm()
...
def comm():
    ...
    if sync:
        sec = os.read(decision_fd, 8)
        sec = int.from_bytes(sec, 'little')
        nsec = os.read(decision_fd, 8)
        nsec = int.from_bytes(nsec, 'little')
        
        release_time = sec + nsec/1000000000
        sync = False
    
    msg = os.read(decision_fd, 4)
    
    msg = int.from_bytes(msg, 'little')
    
    
def msg2bmsg(msg: list):
    bmsg = msg[0].to_bytes(4, byteorder='little')
    for item in msg[1:]:
        bmsg += item.to_bytes(4, byteorder='little')

    return bmsg

def registration(info: list):
    global request_fd
    global decision_fd
    global register_fd
    ...
    info = [1] + info
    binfo = msg2bmsg(info)
    register_fd = os.open("/scheduler", os.O_WRONLY)
    
    os.write(register_fd, binfo)

    while request_fd is None:
        try:
            request_fd = os.open("/tmp/sch_request_{:d}".format(os.getpid()), os.O_WRONLY)
        except:
            pass
    
    while decision_fd is None:
        try:
            decision_fd = os.open("/tmp/sch_decision_{:d}".format(os.getpid()), os.O_RDONLY)
        except:
            pass

def de_registration(info):
    global register_fd
...
class Module:
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        super().__setattr__('priority', None)
        super().__setattr__('pid', None)
        super().__setattr__('period', None)
        super().__setattr__('model_type', None)        
    ...
    def init_sequence(self, ):
        ack = [99]
        back = msg2bmsg(ack)
        os.write(request_fd, back)
        msg = os.read(decision_fd, 4)
        self.cuda()
                
    def rt_swap(self, priority, period, model_type):
        
        ## setting rt_swap related parameters (e.g., rt parameters, ...)
        ...
        ## registration
        print("[RT-Swap] Pid: %d" % self.pid)
        print("[RT-Swap] Period: %d" % self.period)
        print("[RT-Swap] Model Type: %d" % self.model_type)
        info = [self.pid, self.period, self.model_type]
        registration(info)
        
        dummy = torch.Tensor([0]).cuda()
        
        ## making my self as a cuda model as well as booting rt-swap library
        self.init_sequence()
        
        ## comm open register
        self.register_forward_pre_hook(scheduling_interface)
