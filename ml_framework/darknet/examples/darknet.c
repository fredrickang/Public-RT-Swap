...
int main(int argc, char **argv)
{
    ...
    //fprintf(stdout,"Starting %d,%f\n", getpid(), what_time_is_it_now());
    ...
    logdir = find_char_arg(argc,argv, "-logdir",".");
    ...
    period = find_int_arg(argc,argv,"-period", -1);
    model_type = find_int_arg(argc, argv, "-model_type", 0);
    mem_limit = find_size_t_arg(argc, argv, "-mem_limit", 1);
    if(period == -1) fprintf(stderr,"\x1b[31m""Period setting wrong\n""\x1b[0m");
    if(model_type == 0) fprintf(stderr,"\x1b[31m""Model Type setting wrong\n""\x1b[0m");
    if(mem_limit == 1) fprintf(stderr,"\x1b[31m""Mem Limit setting wrong\n""\x1b[0m");
